...
class ObjectsExplorer extends Controller {
  constructor(parent, cfg = {}) {
    ...
    if (!cfg.objectsTabElement) {
      throw "缺少設定: objectsTabElement";
    }

    if (!cfg.showAllObjectsButtonElement) {
      throw "缺少設定: showAllObjectsButtonElement";
    }

    if (!cfg.hideAllObjectsButtonElement) {
      throw "缺少設定: hideAllObjectsButtonElement";
    }

    if (!cfg.objectsElement) {
      throw "缺少設定: objectsElement";
    }
    ...
    this._objectsTabButtonElement = this._objectsTabElement.querySelector(
      ".xeokit-tab-btn"
    );
