...
class ModelsExplorer extends Controller {
  constructor(parent, cfg) {
    ...
    if (!cfg.modelsTabElement) {
      throw "缺少設定: modelsTabElement";
    }

    if (!cfg.unloadModelsButtonElement) {
      throw "缺少設定: unloadModelsButtonElement";
    }

    if (!cfg.modelsElement) {
      throw "缺少設定: modelsElement";
    }
    ...
    this._modelsTabButtonElement = this._modelsTabElement.querySelector(
      ".xeokit-tab-btn"
    );
  ...
  loadModel(modelId, done, error) {
    ...
    this.bimViewer._busyModal.show("正在載入: " + modelInfo.name);
    this.server.getMetadata(
      ...
      (json) => {
        this.server.getGeometry(
          ...
          modelId,
          (arraybuffer) => {
            const objectColorSource =
              modelInfo.objectColorSource ||
              this.bimViewer.getObjectColorSource();
            const objectDefaults =
              objectColorSource === "model"
                ? ModelIFCObjectColors
                : ViewerIFCObjectColors;
            ...
            model.on("loaded", () => {
              ...
              if (this._numModelsLoaded === 1) {
                ...
                this.viewer.cameraControl.pivotPos = math.getAABB3Center(
                  aabb,
                  tempVec3
                );
