...
class ClassesExplorer extends Controller {
  constructor(parent, cfg = {}) {
    ...
    if (!cfg.classesTabElement) {
      throw "缺少設定: classesTabElement";
    }

    if (!cfg.showAllClassesButtonElement) {
      throw "缺少設定: showAllClassesButtonElement";
    }

    if (!cfg.hideAllClassesButtonElement) {
      throw "缺少設定: hideAllClassesButtonElement";
    }

    if (!cfg.classesElement) {
      throw "缺少設定: classesElement";
    }
    ...
    this._classesTabButtonElement = this._classesTabElement.querySelector(
      ".xeokit-tab-btn"
    );
