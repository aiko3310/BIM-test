...
const toolbarTemplate = `<div class="xeokit-toolbar">
    <!-- Reset button -->
    <div class="xeokit-btn-group">
        <button type="button" class="xeokit-reset xeokit-btn fa fa-home fa-2x disabled" data-tippy-content="回到初始視點"></button>
    </div>
...
class BIMViewer extends Controller {
  ...
  _customizeViewer() {
    ...
    scene.highlightMaterial.edgeColor = [0.5, 0.5, 0];
    ...
    const onCameraMatrix = scene.camera.on("matrix", () => {
      ...
      timer =
        saoInteractiveDelay !== null && saoInteractiveDelay !== undefined
          ? this._configs.saoInteractiveDelay
          : 200;
      if (saoEnabled) {
  ...
  setConfig(name, value) {
    ...
    try {
      switch (name) {
        ...
        case "smartPivot":
          this.viewer.cameraControl.smartPivot = this._configs[
            name
          ] = parseBool(value);
          break;

        case "saoEnabled":
          this.viewer.scene.sao.enabled = this._configs[name] = parseBool(
            value
          );
          break;
        ...
        case "saoKernelRadius":
          this.viewer.scene.sao.kernelRadius = this._configs[name] = parseFloat(
            value
          );
          break;
        ...
        case "viewFitFOV":
          this.viewer.cameraFlight.fitFOV = this._configs[name] = parseFloat(
            value
          );
          break;

        case "viewFitDuration":
          this.viewer.cameraFlight.duration = this._configs[name] = parseFloat(
            value
          );
          break;

        case "perspectiveFOV":
          this.viewer.camera.perspective.fov = this._configs[name] = parseFloat(
            value
          );
          break;
        ...
        case "saoInteractiveDelay":
          ...
          if (saoInteractiveDelay < 0) {
            this.error(
              "setConfig() - saoInteractiveDelay cannot be less than zero - clamping to zero"
            );
  ...
  setBackgroundColor(rgbColor) {
    this.viewer.scene.canvas.canvas.style.background =
      "rgba(" +
      rgbColor[0] * 255 +
      "," +
      rgbColor[1] * 255 +
